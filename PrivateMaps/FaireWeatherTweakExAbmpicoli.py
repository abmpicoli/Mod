...
##    print "desired percent",percent
    ...
    while not inTolerance:
        ...
        if(iterations > 500):
            ...
            print (str(threshold)+","+str(thresholdChange))
            break #close enough
        matchCount = 0
...
class ClimateMap :
    ...
    def createClimateMaps(self):
        ...
        for i in range(mc.hmHeight*mc.hmWidth):
            #abmpicoli change: add a random rain factor, to cause some extra noise and less uniform terrain.
            self.moistureMap.append(-0.05+PRand.random()*0.1)
            self.rainFallMap.append(-0.05+PRand.random()*0.1)
...
##        self.printRainFallMap(False)
    ...
    def getInitialTemp(self,x,y,tropic):
        ...
        if aboveSeaLevel:
            ...
            #abmpicoli change: add a small random factor to get less uniform terrain
            temp = 1.0 - mc.oceanTempClamp - (tempPerLatChange * latDifference)-0.05+0.1*PRand.random()
        return temp
...
def getDescription():
	...
	"""
	return "TXT_KEY_MAP_SCRIPT_FW_DESCR_ABMPICOLI_TWEAKS"
...
abmpicoliFaireWeatherDebug=True

abmpicoliLandTerrainTypes = [TerrainTypes.TERRAIN_DESERT,
    TerrainTypes.TERRAIN_PLAINS,TerrainTypes.TERRAIN_PLAINS,TerrainTypes.TERRAIN_PLAINS,
    TerrainTypes.TERRAIN_SNOW,
    TerrainTypes.TERRAIN_TUNDRA,
    TerrainTypes.TERRAIN_GRASS,TerrainTypes.TERRAIN_GRASS,TerrainTypes.TERRAIN_GRASS,
    TerrainTypes.TERRAIN_MARSH,TerrainTypes.TERRAIN_MARSH,
    TerrainTypes.TERRAIN_DESERT,
    TerrainTypes.TERRAIN_SAVANNAH,TerrainTypes.TERRAIN_SAVANNAH,
    TerrainTypes.TERRAIN_TAIGA,TerrainTypes.TERRAIN_TAIGA,TerrainTypes.TERRAIN_TAIGA,
    TerrainTypes.TERRAIN_WETLAND,
    TerrainTypes.TERRAIN_PLAINS_FERTILE,TerrainTypes.TERRAIN_PLAINS_FERTILE
    ]

def abmpicoliPlotTypeToNumber(plotType):
...
def abmpicoliNumberToPlotType(number):
    if number <= 0:
...
# Raises a plot type by the specified amount: OCEAN to LAND or HILLS, LAND to HILLS or PEAKS, HILLS to PEAKS
# Assuming a limitation of amount being 1 or 2.
# Returns NONE is there are no extra options available for this plot
# or the plot itself, after operations done.
def abmpicoliRaisePlot(plot,amount):
    if isAnyAdjacentPlotTerrainType(plot.getX(),plot.getY(),TerrainTypes.TERRAIN_LARGE_RIVERS):
        if abmpicoliFaireWeatherDebug:
            print ("abmpicoli raise plot failure. There is an adjacent large river at " + str(plot.getX())+","+str(plot.getY()))
        return None
    plot_type=abmpicoliPlotTypeToNumber(plot.getPlotType())
    if plot_type == PlotTypes.PLOT_OCEAN:
        
        plot.setPlotType(abmpicoliNumberToPlotType(amount),True,True)
        new_terrain_type_index = PRand.randint(0,len(abmpicoliLandTerrainTypes)-1)
        plot.setTerrainType(abmpicoliLandTerrainTypes[new_terrain_type_index],True,True)
        return plot
    new_plot_type = plot_type + amount
    if new_plot_type > 3:
        if abmpicoliFaireWeatherDebug:
            print ("abmpicoli raise plot failure. feature too high: " + str(new_plot_type) +": "+ str(plot.getX())+","+str(plot.getY()))
        return None
    new_plot_type_enum = abmpicoliNumberToPlotType(new_plot_type)
    if abmpicoliFaireWeatherDebug:
        print ("abmpicoli lowerplot new_plot_type_enum="+str(new_plot_type_enum)+": actual value = " + str(new_plot_type))
    plot.setPlotType(new_plot_type_enum,True,True)
    return plot


# Lowers a terrain by a specified amount. LAND to OCEAN, HILLS to LAND, PEAKS to HILLS or LAND, etc.
# assumes amount is either 1 or 2.
def abmpicoliLowerPlot(plot,amount):
    if isAnyAdjacentPlotTerrainType(plot.getX(),plot.getY(),TerrainTypes.TERRAIN_LARGE_RIVERS):
        if abmpicoliFaireWeatherDebug:
            print ("abmpicoli lower plot failure. There is an adjacent large river at " + str(plot.getX())+","+str(plot.getY()))
        return None
    plot_type=abmpicoliPlotTypeToNumber(plot.getPlotType())
    new_plot_type = plot_type - amount
    if new_plot_type < 0:
        if abmpicoliFaireWeatherDebug:
            print ("abmpicoli lower plot failure. feature too low: " + str(new_plot_type) +": "+ str(plot.getX())+","+str(plot.getY()))
        return None
    new_plot_type_enum = abmpicoliNumberToPlotType(new_plot_type)
    ...
    if abmpicoliFaireWeatherDebug:
        print ("abmpicoli lowerplot new_plot_type_enum="+str(new_plot_type_enum)+": actual value = " + str(new_plot_type))
    plot.setPlotType(new_plot_type_enum,True,True)
    if new_plot_type == PlotTypes.PLOT_OCEAN:
        plot.setTerrainType(TerrainTypes.TERRAIN_COAST,True,True)
    return plot

# perform corrections in the landscape, adjusting too flat terrains or too hilly (near unpassable) terrain.
# mountain chains will be broken in this situation, without leaving big linear mountain chains.
def abmpicoliAdjustments():                        
        ...
        while limit % prime == 0 or limit % 3 == 0:
            prime += 2
        if abmpicoliFaireWeatherDebug:
            print ( "abmpicoli change v2025-05-03 124700" )
            print ( "abmpicoli peak="+str(PlotTypes.PLOT_PEAK)+": hills=" + str(PlotTypes.PLOT_HILLS) + ": land=" + str(PlotTypes.PLOT_LAND) +": ocean = " + str(PlotTypes.PLOT_OCEAN) )
        ...
        for index in range(limit):
            ...
            x = i % mc.width
            y = i // mc.width
            if x <=0 or x>=mc.width-1 or y <=0 or y >= mc.height-1:
                continue
            issue_score=0
            plot = mmap.plot(x,y)
            ...
            if plot.getPlotType() != PlotTypes.PLOT_OCEAN:
                ...
                for dx in range(-1,2,1):
                    ...
                    for dy in range(-1,2,1):
                        ...
                        x2=x+dx
                        y2=y+dy
                        if x2 < 0 or x2 >= mc.width:
                            continue
                        if y2 < 0 or y2 >= mc.height:
                            continue
                        thisPlot = mmap.plot(x2,y2)
                        if thisPlot.getTerrainType() == terrainLargeRiver:
                            bypass=True
                            break
                ...
                while ( issue_score < min_score or issue_score > max_score ) and attempts < 18:
                    ...
                    issue_str=None
                    attempts += 1
                    if issue_score < min_score:
                        adjustment=abmpicoliRaisePlot
                        issue_str="too flat"
                    else:
                        adjustment=abmpicoliLowerPlot
                        issue_str="too high"
                    amount=1
                    ...
                    if abmpicoliFaireWeatherDebug:
                        print ( "abmpicoli "+ issue_str + " neighborhood at plot "+str(x)+","+str(y)+": issue score="+str(issue_score)+ " attempts="+str(attempts) +" amount to try:" + str(amount) )
                    the_plot_to_change_index=PRand.randint(0,len(directions) - 1)
                    while directions[the_plot_to_change_index] is None and attempts < 18:
                        attempts += 1
                        the_plot_to_change_index = (the_plot_to_change_index + 1) % len(directions)
                    the_plot_to_change = directions[the_plot_to_change_index]
                    if the_plot_to_change is None:
                        attempts = 18
                    else:
                        old_plot_type = abmpicoliPlotTypeToNumber(the_plot_to_change.getPlotType())
                        old_terrain_type = the_plot_to_change.getTerrainType()
                        change_x = the_plot_to_change.getX()
                        change_y = the_plot_to_change.getY()
                        the_plot_to_change = directions[the_plot_to_change_index] = adjustment(the_plot_to_change,amount)
                        
                        if not ( the_plot_to_change is None ) :
                            new_plot_type = abmpicoliPlotTypeToNumber(the_plot_to_change.getPlotType())
                            issue_score = issue_score + new_plot_type * new_plot_type - old_plot_type * old_plot_type
                            if abmpicoliFaireWeatherDebug:
                                print ( "abmpicoli new plot at plot "+str(change_x)+","+str(change_y) + " from " + str(old_plot_type) +" to " + str(new_plot_type) + ": attempts="+str(attempts)+" issue_score="+str(issue_score) )
                        
                    if attempts >= 18:
                        if abmpicoliFaireWeatherDebug:
                            print ( "abmpicoli found no solution for plot " + str(x)+","+str(y)+": issue score=" + str(issue_score) )


def afterGeneration():

    abmpicoliAdjustments()
